  ...
  template: `
    ...
    <div class="space-y-6">
      ...
      <ui-callout title="The agent has to agree on the state shape">
        ...
        <code>{{ '{' }} notes, priority {{ '}' }}</code>. On the Agent Framework
        side the shape is declared by the <code>state_schema</code> passed to
        <code>AgentFrameworkAgent</code>, with
        <code>predict_state_config</code> mapping each key to the tool argument
        that updates it. The agent in <code>backend/main.py</code> currently
        declares <code>language</code>, not
        <code>{{ '{' }} notes, priority {{ '}' }}</code>, so until the schemas
        are aligned the panel sits on its <code>EMPTY_STATE</code> defaults.
      </ui-callout>
