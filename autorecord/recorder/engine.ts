...
export class RecordingEngine {
  ...
  async recordPage(
    ...
    if (!health.frontendOk || !health.runtimeOk || !health.backendOk) {
      ...
      if (!health.backendOk) {
        console.warn(
          `   🔴 Microsoft Agent Framework Backend (port 8200) is unreachable: ${health.backendError}`,
        );
        console.warn(
      ...
      if (!health.runtimeOk) {
        console.warn(
          `   🔴 Copilot Runtime Node.js (port 8201) is unreachable: ${health.runtimeError}`,
        );
        console.warn(
      ...
      console.log(`   ✅ Pre-flight checks passed: Angular (:4200), Runtime (:8201), Python (:8200) are reachable.`);
    }
    ...
    try {
      if (config.docOnly) {
        // ----------------------------------------------------
        // DOC-ONLY PRESENTATION MODE (stays on documentation page)
        // ----------------------------------------------------
        console.log(
        ...
        try {
          ...
          // Dispatch doc-only presentation action (e.g. runA2uiAction)
        ...
        if (config.id === 'quickstart') {
          console.log(
            `\n💻 Step 2a: Displaying CopilotKit & AG-UI Versions in package.json (lines 20-35)...`,
          );
          try {
